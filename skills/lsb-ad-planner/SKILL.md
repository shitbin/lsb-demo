...
> No prior context? Read `../GLOSSARY.md` first — it defines every shared term (brief, dataset, cross-pollination, hero cut, moodboard library, etc.).
...
**(A) Global meta (same keys as treatment_global_template):** brand · product · target_demo / total_duration_sec · shot_count · fps · aspect_ratio · hook_position_sec · cta_position_sec / narrative_arc · pacing_curve · music_tempo_curve · wow_cut_index[] · creative_device / production_signature.* / global_layout.* / recurring_motifs[] / typography_global.* / vfx_global.* / copywriting.* / **client_perception_path (7-step emotion, exactly as STEP 3 B)** / **strategy_spine (the 6 plan-logic fields brief · insight + evidence · strategy · concept_rationale · brand_right · payoff, exactly as STEP 3 B — the builder renders it as the STRATEGY section)** / **narrative_structure (STEP 3 (C))** / **character_pool[] (exactly as STEP 3 (C-2) if 2+ people or cross-cutting)** / audio_intent.bgm/narration / **`moodboard_bucket`** (the 1–2 bucket key(s) picked in STEP 0.55, e.g. `05_필름그레인_인물무드` — feeds lsb-image-crafter gate G6, which uploads representative bucket images as visual references) / `photographic_treatment` (the global tone → P1–P6 mapping).
...
- Typography & subtitles: copy_overlay (verbatim on-screen copy), **`typo_mode`** (none / subtitle / baked — whether this frame's copy is rendered INTO the image vs composited in post; **main copy / headline / CTA / emphasis = baked, Korean included**; subtitle only for long running subtitle lines / legal), **`baked_text`** (the exact string to bake = copy_overlay when typo_mode=baked, else null), layout_grid, subject_typo_layout, typo_motion, typo_color_strategy. *(typo_mode + baked_text feed lsb-image-crafter gate G5 — required on every cut.)*
- VFX: vfx_in_shot[], vfx_intensity_local, vfx_in_board_prompts
- Audio: audio_intent.sfx / narration_line / bgm_change / silence
- Visual inventory: visible_elements (foreground/midground/background/lighting_env/atmosphere — required for image-crafter G4), texture, lighting, color_analysis, style_prompt, **`photographic_preset`** (P1–P6, tone-mapped from the global `photographic_treatment` — feeds lsb-image-crafter gate G7)
...
**(B) Downstream input format — ONE plan JSON, consumed by two skills:**
```json
{ "global": { /* treatment_global_template */ }, "cuts": [ /* cut_template array */ ], "transitions": [ /* transition_template array */ ] }
```
This JSON is the downstream Phase 0 input as-is. To pass, the user must trigger the next skill *without edits*.
- **lsb-image-crafter (runs first) reads the per-cut VISUAL fields as its `cut_plan.json`:** `composition`
  (1–2 of the 7 laws) · `eye_path` · `photographic_treatment`/preset (P1–P6) · `visible_elements`
  (foreground/midground/background/lighting_env/atmosphere) · `moodboard_bucket` (the bucket picked in
  STEP 0.55) · **`typo_mode` (none/subtitle/baked) + `baked_text` (verbatim on-image copy)** per cut and
  for the KV · `subject_identity`/`requires_character_sheets`. **These feed image_crafter's gates G2–G7,
  so they are REQUIRED on every cut** (a cut missing `composition` or `visible_elements` or `typo_mode`
  fails the gate). The contract: `lsb-image-crafter/REFERENCE/data-contract.md`.
- **lsb-treatment-builder reads the strategy/copy/layout fields** + the `stills.json` image_crafter
  produced, and lays out the deck. It does not generate images.
This JSON is the builder's Phase 0 input as-is. To pass, the user must be able to trigger the next skills *without edits*.
