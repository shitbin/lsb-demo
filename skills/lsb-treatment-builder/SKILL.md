...
> No prior context? Read `../GLOSSARY.md` first — it defines every term used here
...
- **lsb-image-crafter**: the single owner of all image generation (master sheets, key visual, cut stills). It runs BEFORE this skill and outputs `stills.json` + verified local stills.
- **lsb-treatment-builder (this skill)**: takes the plan + **`stills.json` (the already-generated images)** and lays out the storyboard and deck pages into a PDF. **It does NOT call `generate_image`** — image generation moved to lsb-image-crafter (260614 split). Phase 2/3 below are retained only as the legacy spec / fallback when stills.json is absent.
...
1.5 **`stills.json` + `assets/` (★ from lsb-image-crafter — the already-generated images)** — the master sheets, key visual, and per-cut stills the image-crafter produced (`stills.json` lists `cuts[].path`, the KV path, master-sheet paths, and the `declined_preset_ids`). **This skill does NOT generate images** — load `stills.json` here and map `cuts[].path` into the deck at Phase 4. If `stills.json` is absent (a standalone run with no image stage), and only then, fall back to the LEGACY Phase 2/3 generation path below.
...
3. **Dataset (optional)** — `entries/` + `index/` inside `001_ad_video_dataset/` (= `<DATASET>`) inside `<LIBRARY>` = the folder that **directly contains `001_ad_video_dataset/`** (resolve by structure: take the connected folder; if it directly contains `001_ad_video_dataset/` that IS `<LIBRARY>`, else drop into its `library/` subfolder — **API:** the `shitbin/lsb-demo` repo → its `library/`; **Local (Mac Cowork):** `/Users/soobin/Desktop/LSB_AD_ENGINE/library`). **Do not hardcode paths** — resolve at runtime (works on both mac `/Users/...` and win `C:\Users\...`). If present, use it for cross-pollination; if absent, pick directly from the catalog.
...
### Phases 2 & 3 — ★ MOVED to lsb-image-crafter (image generation is no longer this skill's job)

> **All image generation — master character sheets (Phase 2), scene/transition boards and cut stills
> (Phase 3), slicing — now lives in `lsb-image-crafter`, which runs before this skill and enforces the
> 7 gates (declined_preset · full-field cells · composition · fg/mg/bg · baked-copy/no-"no text" ·
> moodboard refs · model/preset lock). This skill CONSUMES the result: read `stills.json`
> (`assets/확정컷/cutNN.png`, the KV, the master sheets) and go straight to Phase 4 layout. Do NOT call
> `generate_image` here. The Phase 2/3 text below is kept only as the legacy spec and as a fallback if a
> session runs this skill standalone with no stills.json.**

### Phase 2 — Master character sheet [LEGACY · see lsb-image-crafter]
...
**4.0 Build order — skeleton first, insert the stills when they land (★ overlap the image wait, never ship a partial PDF).**
The image stage (lsb-image-crafter) renders asynchronously, so do **not** idle-wait for it. Build in two passes:
- **Pass A — skeleton (runs WHILE images are still generating):** everything that does NOT need the pixels —
  the page plan + archetype choice (§4.2 structure), all typeset copy · captions · strategy beats · section
  labels (§4.2-a/-b), the brand theme, and each image slot's coordinates **sized to that cut's planned
  ratio** (from `cut_plan`/`treatment.json`). Reserve the slots; do **not** fill them with code-drawn shapes.
- **Pass B — insert + finalize (ONLY after `stills.json` is complete):** when every required still is
  downloaded + verified, `T.place_image` each `stills.json` `cuts[].path` / `key_visual.path` /
  `master_sheets[].path` into its reserved slot, run the Phase 5 gates, then render the final PDF.
- **★ Never emit `pdf_ready` or call the deck "완성 / done" until Pass B has filled EVERY image slot with a
  real verified still** (Phase 5 gate ⑩ — 0 placeholders on any image page). A skeleton, or any
  empty-slot/placeholder PDF, is an **internal intermediate** — never shared, never announced as the
  treatment. If a `stills.json` path is missing/broken, **hold** and tell the user (the image stage re-runs);
  do not finalize a partial deck.
...
- **★ Source the images from `stills.json` (Phase 0 item 1.5).** Read `stills.json`: map each `cuts[].path` to its deck cut, the `key_visual.path` to the cover/KV, and the `master_sheets[].path` where a character page is needed. Pass these local paths into `T.place_image` / `T.storyboard_grid` / `T.cut_board`. Cut-board / storyboard pages = the v12 archetypes (centered, readable captions). Do not call `generate_image` and do not draw code-vector substitutes; if a `stills.json` path is missing/broken, stop and tell the user (the image stage must re-run), don't patch with shapes.
...
- **★ THE IMAGE LAW (the #1 sin a prior gen broke · absolute):** `T.place_image`'s real modes are
  **`auto` (default)** / `contain` / `cover` (the words `fit`→`contain` and `fill`→`cover` are accepted
  aliases). `auto` = no crop unless the source already matches the cell ratio within ±8% (then a true
  cover), else letterbox `contain`. Scenes, mood, storyboard cuts, supporting cuts — all **in their
  original ratio**, no cropping / distortion / arbitrary stretching → use `auto`/`contain`. `cover`
  (fill the frame) is allowed **only for cover/closing KV, and only when the source ratio is within
  ±8% of the page's 16:9** (otherwise `auto` already falls back to `contain`). **Do not crop a vertical (9:16) source horizontally** — stand it up as a vertical card. Rounded corners / shadows / borders default to 0 (a card exception applies only to the luxury tone).
...
- Archetypes (§4, 12 types): `cover_film`·`cover_type`·`section_divider`·`concept_headline`·`narration_still`·`mood_board`·`scene_hero`·`scene_cluster`·`storyboard_grid`·`cut_board`·`option_ab`·`closing`.
- **★ Cut dict keys = EXACTLY what the functions read (don't pass `caption`/`na` — they're silently dropped):**
  - `storyboard_grid(cuts=[…])` reads **`{img, no, tc, cap, ref}`** — `cap` is the one-line action (NOT `caption`), `tc` the timecode, `img` the local still path from stills.json, `ref` a bool.
  - `cut_board(cuts=[…])` reads **`{img, no, name, scene, vo}`** — `name` = scene title / headline, `scene` = the scene description, `vo` = the voice-over line.
  - **Map from the plan/stills when you build these dicts:** plan `copy_overlay`/one-line action → `cap`; cut duration → `tc` (e.g. "00:00-00:03"); stills.json `cuts[].path` → `img`; plan scene title → `name`; plan scene description → `scene`; plan `voiceover`/`narration_line` → `vo`. (These are positional dict keys for the PIL functions, not the planner schema names — translate explicitly.)
- **★ Cut-board / storyboard layout law (fixed defects):** full-still per-cut pages MUST use `T.cut_board(...)` (≤2 cuts/page; readable caption scale — scene name ~30 / `장면`·`V.O` ≥20). Do NOT hand-roll a 2-up with tiny 13–16px captions and a top-heavy layout. `T.storyboard_grid(...)` now CENTERS the grid (equal L/R margins, each row by its own item count, vertically balanced) — never left-pinned with dead right space. Caption fonts in both ≥16, cut-number ≥18.
...
**⑩ Image mandate gate (required · code-enforced):** on cover, key-visual, cut-board, and scene pages call `T.assert_images_present(placed_flags, page_kind)` — if 0 real composited images (placeholders only), hold the build. Blocks a flat slide that's "just text on a gradient." **★ Completion rule (4.0 Pass B): EVERY image slot across all image pages must hold a real verified still — 0 remaining reserved/placeholder slots — before the deck is output. While any cut/KV/master-sheet image is still generating, or any `stills.json` path is missing/broken, HOLD: do not render the final PDF, do not emit `pdf_ready`, do not say "done." A skeleton/partial-image PDF is an internal intermediate, never the deliverable.**
