...
리디자인급(좌측정렬 위계·2단·풀블리드·썸네일 그리드)을 기본값으로. ★ 텍스트 박스(크림/미색 채운 카드) 금지 — 텍스트는 다크 배경 위 직접(_2606110030 디렉터 지시).
...
# 기본 다크 에디토리얼 테마. 브랜드별로 덮어쓴다.
# ★ 텍스트 뒤 채운 면(카드/박스) 금지 — 디렉터 확정 지시(_2606110030). 텍스트는 bg 위에 직접.
#   surface = bg보다 살짝 밝은 동계열 다크 면 — '이미지 플레이스홀더 등 비(非)텍스트 면' 전용.
#   크림/미색(#FBF7EF 류) 면을 텍스트 배경으로 쓰지 않는다. 페이지 호흡 = 여백·명도 리듬·헤어라인.
THEME_EDITORIAL = {
    "bg": "#2A0A12", "surface": "#3A1C23", "surface_ink": "#F5EEE7",
    "ink": "#F5EEE7", "muted": "#C8B6A8", "point": "#C9882F", "line": "#5A2E1A",
    # typeset 호환 키
    "text": "#F5EEE7",
}
def theme_for_typeset(theme, on_surface=False):
    """typeset()/draw_block은 text/point/muted 키를 본다. (surface도 다크 — 잉크는 밝은 색 유지.)"""
    if on_surface:
...
# ── 아키타입 B: 2단 (논증 좌 / PROOF 직접 조판 또는 이미지 우 · 텍스트 박스 없음) ──
def two_col(im, draw, theme, headline, body="", eyebrow_num=None, eyebrow_txt="STRATEGY",
    ...
    if side_img:
        ...
        # ★ 텍스트 박스 금지(디렉터 지시 _2606110030): 채운 카드 대신 헤어라인 + 배경 위 직접 조판
        px0, py0, px1, py1 = S(rx0), S(MARGIN+40), S(PW-MARGIN), S(PH-MARGIN-40)
        blocker = [(px0, py0, px1, py1)]
        draw.line([px0, py0, px0, py1], fill=_rgb(theme["line"]), width=S(2))
        f0 = Ft(_BODYF[0], 18)
        draw.text((px0+S(48), py0+S(8)), " ".join(list(proof_title)),
                  font=f0, fill=_rgb(theme["point"]))
        cy = py0+S(62)
        for p in (proof_points or []):
            draw.ellipse([px0+S(48), cy+S(10), px0+S(60), cy+S(22)], fill=_rgb(theme["point"]))
            _para(draw, p, px0/SCALE+76, cy/SCALE, (px1-px0)/SCALE-108, theme,
                  size=23, color=theme["ink"])
            cy += S(64)
    footer(draw, theme, left="LSB PRODUCTION", right="AD TREATMENT")
