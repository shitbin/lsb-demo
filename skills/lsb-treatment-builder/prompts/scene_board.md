...
> No prior context? See ../../GLOSSARY.md and REFERENCE/photographic-treatment.md.
...
3. **Attach the master character sheet as a reference.**
 - This is the foundation of character consistency (the protagonist looking like the same person in
   every panel). Omit it and a different person shows up each time. (Do not photo-realistically clone a
   real celebrity's face — the master sheet is the studio's own model / talent type.)
...
7. **Describe each panel with a structured set of fields (most important).**
 ...
 - For each panel, stamp in ALL of: framing / camera_angle / camera_facing / shot_scope / subject_action
   / pose / gaze / props / layout_grid / color_intent / vfx — using the **analyzer vocabulary (English
   tokens)**.
...
## On-screen typography — baked vs subtitle

On-screen text that is **baked** = headline / emphasis / kinetic (kinetic typography) goes INTO the
image (`typo_mode=baked`). Only **subtitles** are added in post-processing (`typo_mode=subtitle`). In the
board, draw baked typography as part of the image; keep the verbatim subtitle text in the panel
description / plan but composite it later.
...
---
*Version: lsb-treatment-builder_2606140000 · 2026-06-02 15:05 KST.*
*_2606140000 = English rewrite + photographic-treatment presets wired into the Style block.*
