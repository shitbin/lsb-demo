...
**가드:** 긴 문장·약관·정밀 수치·표는 baked 금지 → subtitle(후처리). negative에 `garbled text, extra letters, misspelled` 넣어 깨짐 억제(단 텍스트 자체는 허용 — 기존 전면 "no text" 금지는 해제). 텍스트가 깨지면 더 짧게/영문으로/재생성.
...
4. **깨짐**: baked 텍스트가 garble? → 짧게/영문/재생성. 정밀 수치는 후처리.
