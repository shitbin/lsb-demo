...
| 레이아웃 | 전부 중앙정렬 텍스트 덤프 | **좌측정렬 위계 + 2단(텍스트+카드/이미지)** |
| 팔레트 면 | 전 페이지 다크 BG | 정보 페이지는 **크림 surface**, 임팩트만 다크/풀블리드 |
| 컷 개관 | 이미지 1장 + 여백 | **컷보드 = 썸네일 그리드 1장** |

이 5축이 이 시스템의 강제 항목이다.
...
surface     크림/라이트 면 — 카드·보드·정보 페이지 배경 (예 #FBF7EF)
surface_ink 크림 위 본문 잉크 (다크)
...
- 크림(surface) 위에 텍스트를 쓸 땐 `theme_for_typeset(theme, on_surface=True)`로 잉크색 전환.
- 다크/크림 **교대**로 리듬: 표지(다크) → 전략(다크 or 크림 카드) → 컷보드(크림) → KV(풀블리드) → … 정보밀집=크림, 임팩트=다크.
...
| `two_col(im,d,theme,headline,body,eyebrow_num,eyebrow_txt,proof_points=...,side_img=...)` | 전략·논증·비교 | 좌: eyebrow(번호)→headline→body / 우: **크림 PROOF 카드(불릿)** 또는 이미지. 겹침 게이트 내장 |
...
- 모든 아키타입은 `rounded_img()`로 이미지를 **cover-fit + 라운드** 합성; 이미지 없으면 크림 플레이스홀더(실빌드에선 금지 → ⑩ 게이트).
...
- [ ] 정보 페이지에 크림 surface로 호흡? 전부 다크 아님? (⑫)
