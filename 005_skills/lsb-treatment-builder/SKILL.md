...
- **§4.3 Cross-pollination**: 비주얼 톤 레퍼런스를 끌어올 때 가중치는 **planner와 동일 프로파일** — 현재 LOOSE 테스트: 0.5(동일)/0.8(인접)/1.0(원거리)/1.1(대조)·와우컷 동일 0.3 약참조 (STRICT 복원값: 0.2/0.5/1.0/1.2·와우컷 하드밴). (판정 맵 단일 출처 = `lsb-ad-planner/schema.md` §3, 영문 industry 기준. 프로파일 정의 = planner SKILL.md cross-pollination 절.)
...
**원칙:** 픽셀을 베끼지 않는다. 끌어오는 건 추상화된 시그니처(레이아웃 그리드·타이포 무게·컬러 의도·슬라이드 흐름) + 시각 인벤토리/recreation_prompts(영감으로). **가중치**(§4.3, planner와 동일 프로파일 — 현재 LOOSE): 동일 0.5 / 인접 0.8 / 원거리 1.0 / 대조 1.1 · 와우컷 동일 0.3 (STRICT 복원값: 0.2/0.5/1.0/1.2·하드밴). 판정 맵 단일 출처 = `lsb-ad-planner/schema.md` §3.

**예 (LOOSE 기준):** 통신사 트리트먼트면 → 통신(0.5) / 은행·보험(0.8) / 패션·뷰티 KV(1.0) / 산업 B2B(1.1).
