...
- `generate_video`를 **t2v로 호출**(start_image 없이 텍스트 프롬프트 주도). 프롬프트는 STEP 4 그대로 **4000단어 풀스펙**(스틸이 없으니 공간·동작·인물·레이아웃 묘사를 더 촘촘히).
...
## STEP 4 — 프롬프트 작성
- **분량 (★ 하드 최소 4000단어 · 미달 제출 금지 · 무시하지 말 것):** 모든 i2v/t2v 프롬프트는 **15초 클립 기준 최소 4000단어**다. 구성: 원테이크/컷 선언 → 캐릭터 락 → 컬러·그레이드 → 비트별 타임코드 모션·카메라·트랜지션 → 사운드 → 한국어 나레이션 → **브랜드 모션타이포** → 텍스트 규칙 → 네거티브 → 아트디렉션. **제출 직전 프롬프트의 단어 수를 직접 세고, 4000단어 미만이면 비트 묘사를 더 채워 4000을 넘긴 뒤에만 제출한다(미달 상태로 절대 제출 금지).** 몽타주로 쪼갠 짧은 세그먼트(4~5초)도 *같은 밀도*로 쓰되 최소 1500단어. 한두 문장짜리 빈약한 프롬프트는 컷 누락·저품질의 직접 원인 — 이 규칙을 건너뛰지 않는다.
...
- **브랜드 모션타이포(슬로건/로고/숫자)가 실제로 화면에 떴나? (자주 누락 — 없으면 그 클립 재생성)**
- **클립 이음매가 seamless한가(점프·튐 없이)?**
